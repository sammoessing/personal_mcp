...
async function discordFetch(actor: ConnectorActor, path: string) {
  const token = await getConnectorAccessToken(actor, "discord");
...
export const discordTools: ToolDefinition[] = [
  {
    ...
    handler: async (_args: unknown, ctx: ToolContext) => {
      const guilds = (await discordFetch(ctx, "/users/@me/guilds")) as Array<{ id: string; name: string }>;
  ...
  },
  {
    ...
    inputSchema: {},
    handler: async (_args: unknown, ctx: ToolContext) => {
      const me = (await discordFetch(ctx, "/users/@me")) as { username: string; id: string; email?: string };
