...
/** Object keys are always '<workspace_id>/<uuid><ext>' — see the storage policies. */
function objectKey(workspaceId: string, filename: string): string {
  ...
  return `${workspaceId}/${randomUUID()}${safeExt}`;
}
 ...
 */
export async function createUploadTargetAction(filename: string, sizeBytes: number) {
  if (sizeBytes > MAX_FILE_BYTES) {
    throw new Error("That file is larger than the 50 MB limit.");
  }

  const ws = await requireCurrentWorkspace();
  const path = objectKey(ws.id, filename);
